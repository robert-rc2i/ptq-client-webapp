        ...
        <Modal show={show} onHide={handleClose}>
            ...
            <Modal.Body>
                <RenderBasedOnApiVersion requiredVersion="7.5.3" currentVersion={ctx.ptqInfo.version}>
                    ...
                    <RangeViewController label="BPM" labelValue={false} name="bpm" min={24} max={360} step={1} value={metronomeState.bpm} dispatch={reducer} onChange={(v) => reducer({ type: "setMetronome", value: { ...metronomeState, bpm: v } })} apiCallback={(v, r) => { PtqApi.setMetronome({ ...metronomeState, volume_db: v }, r) }} />
                    <RangeViewController label="Volume" name="volume" min={-35} max={20} step={1} value={metronomeState.volume_db} dispatch={reducer} onChange={(v) => reducer({ type: "setMetronome", value: { ...metronomeState, volume_db: v } })} apiCallback={(v, r) => { PtqApi.setMetronome({ ...metronomeState, bpm: v }, r) }} />
    ...
    return (
        <Dropdown onSelect={(ts) => { PtqApi.setMetronome({ ...metronome, timesig: ts }, reducer) }}>
            ...
            <Dropdown.Menu>
                ...
                <Dropdown.Item eventKey="12/8" active={"12/8" === metronome.timesig} disabled>12/8</Dropdown.Item>
            </Dropdown.Menu>
        </Dropdown>
    )
} 